...
export default function App() {
 ...
 if (state.screen === 'setup') return (
    <div style={{ minHeight:'100vh', background:'#080c0a', fontFamily:'JetBrains Mono, monospace', display:'flex', flexDirection:'column' }}>
      ...
      {/* HERO with map/radar background */}
      <div style={{ position:'relative', overflow:'hidden', borderBottom:'0.5px solid #0f1f14', minHeight:280 }}>

        {/* Grid background */}
        <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(29,158,117,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(29,158,117,0.05) 1px, transparent 1px)', backgroundSize:'40px 40px', pointerEvents:'none' }}/>

        {/* SVG map + radar */}
        <svg style={{ position:'absolute', right:0, top:0, width:'55%', height:'100%', opacity:0.18 }} viewBox="0 0 700 300" preserveAspectRatio="xMidYMid slice">
          {/* US outline — simplified path */}
          <path d="M80,120 L95,100 L120,95 L140,90 L180,85 L220,82 L260,80 L300,78 L340,80 L380,82 L420,85 L450,88 L470,95 L480,108 L475,120 L460,130 L440,140 L420,148 L400,155 L380,160 L360,165 L340,168 L320,170 L300,172 L280,170 L260,168 L240,165 L220,160 L200,155 L180,148 L160,140 L140,132 L120,128 L100,128 Z" fill="none" stroke="rgba(29,158,117,0.6)" strokeWidth="1"/>
          {/* State grid lines */}
          {[100,140,180,220,260,300,340,380,420].map(x => (
            <line key={x} x1={x} y1="80" x2={x} y2="172" stroke="rgba(29,158,117,0.15)" strokeWidth="0.5"/>
          ))}
          {[100,120,140,160].map(y => (
            <line key={y} x1="80" y1={y} x2="480" y2={y} stroke="rgba(29,158,117,0.15)" strokeWidth="0.5"/>
          ))}
          {/* City dots */}
          {[[280,125],[350,118],[420,130],[200,138],[450,95],[150,120],[310,160],[240,110]].map(([x,y],i) => (
            <circle key={i} cx={x} cy={y} r="2" fill="rgba(29,158,117,0.6)"/>
          ))}
          {/* Connection lines */}
          {[[280,125,350,118],[350,118,420,130],[280,125,200,138],[350,118,310,160]].map(([x1,y1,x2,y2],i) => (
            <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(29,158,117,0.2)" strokeWidth="0.5"/>
          ))}
          {/* Radar sweep origin */}
          <circle cx="520" cy="150" r="80" fill="none" stroke="rgba(29,158,117,0.15)" strokeWidth="0.5"/>
          <circle cx="520" cy="150" r="55" fill="none" stroke="rgba(29,158,117,0.12)" strokeWidth="0.5"/>
          <circle cx="520" cy="150" r="30" fill="none" stroke="rgba(29,158,117,0.1)" strokeWidth="0.5"/>
          <circle cx="520" cy="150" r="8" fill="rgba(29,158,117,0.3)"/>
          {/* Radar sweep line — static approximation */}
          <line x1="520" y1="150" x2="600" y2="90" stroke="rgba(29,158,117,0.4)" strokeWidth="1"/>
          <path d="M520,150 L600,90 A80,80 0 0,1 580,220 Z" fill="rgba(29,158,117,0.04)"/>
          {/* Blip dots on radar */}
          <circle cx="545" cy="125" r="2.5" fill="rgba(29,158,117,0.7)"/>
          <circle cx="490" cy="140" r="2" fill="rgba(29,158,117,0.5)"/>
          <circle cx="560" cy="170" r="1.5" fill="rgba(29,158,117,0.4)"/>
        </svg>
        ...
        {[{top:10,left:10,borderWidth:'1px 0 0 1px'},{top:10,right:10,borderWidth:'1px 1px 0 0'},{bottom:10,left:10,borderWidth:'0 0 1px 1px'},{bottom:10,right:10,borderWidth:'0 1px 1px 0'}].map((s,i) => (
          <div key={i} style={{ position:'absolute', width:16, height:16, borderStyle:'solid', borderColor:ac, opacity:0.5, ...s }}/>
        ))}

        {/* Top status bar */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 24px', borderBottom:'0.5px solid #0f1f14', background:'rgba(8,12,10,0.9)', position:'relative', zIndex:2 }}>
        ...
        {/* Hero content + floating jurisdiction panel */}
        <div style={{ display:'grid', gridTemplateColumns:'1fr 220px', gap:24, padding:'24px 32px 24px', position:'relative', zIndex:1, maxWidth:1200, margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
          ...
          <div>
            ...
            <p style={{ fontSize:11, color:'#4a8a68', lineHeight:1.8, margin:'0 0 20px', maxWidth:500, letterSpacing:'0.02em' }}>
            ...
            <div style={{ display:'flex', gap:32 }}>
              {[['TRAIN','Realistic scenarios built for your role'],['DECIDE','Sharpen judgment under pressure'],['COMMAND','Practice ICS doctrine'],['REVIEW','AI after-action every scenario']].map(([label,desc]) => (
                <div key={label} style={{ display:'flex', flexDirection:'column', gap:4 }}>
                  ...
                  <span style={{ fontSize:9, color:'#4a7a5a', lineHeight:1.5 }}>{desc}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Right: floating jurisdiction + system panel */}
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {/* System stats */}
            <div style={{ border:'0.5px solid #1a3028', borderRadius:3, background:'rgba(8,12,10,0.85)', padding:'10px 12px' }}>
              {[['SCENARIOS','10 LOADED'],['JURISDICTIONS','6 TYPES'],['DIFFICULTY','5 LEVELS'],['AAR','AI-POWERED'],['VERSION','2.0']].map(([k,v]) => (
                <div key={k} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'4px 0', borderBottom:'0.5px solid #0f1f14' }}>
                  <span style={{ fontSize:8, color:'#4a7a5a', letterSpacing:'0.1em' }}>{k}</span>
            ...
            {state.jurisdiction && (
              <div style={{ border:`0.5px solid #1a3028`, borderLeft:`3px solid ${ac}`, borderRadius:3, background:'rgba(8,12,10,0.9)', padding:'10px 12px' }}>
                <div style={{ fontSize:8, color:ac, fontWeight:700, letterSpacing:'0.12em', marginBottom:6, textTransform:'uppercase' }}>{state.jurisdiction}</div>
                <div style={{ fontSize:9, color:'#5aaa80', lineHeight:1.7, marginBottom:6 }}>{JURISDICTION_CONTEXT[state.jurisdiction].desc}</div>
                <div style={{ fontSize:8, color:'#4a8a68', lineHeight:1.6, paddingTop:6, borderTop:'0.5px solid #1a3028' }}>
                  <span style={{ color:ac, fontWeight:700 }}>Constraints: </span>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px 32px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', width:'100%', boxSizing:'border-box' }}>
            ...
            {Object.entries(SCENARIOS).map(([key, sc]) => {
              ...
              return (
                <button key={key} onClick={() => update({ scenario:key })}
                  style={{ textAlign:'left', padding:'14px 12px', border:`0.5px solid ${selected?ac:'#1a2e20'}`, borderLeft:`${selected?'3px':'0.5px'} solid ${selected?ac:'#1a2e20'}`, background:selected?ac+'14':'#0c120e', cursor:'pointer', borderRadius:3, outline:'none', position:'relative' }}
                  ...
                  <div style={{ fontSize:10, fontWeight:700, color:selected?ac:'#5a9a70', marginBottom:5, letterSpacing:'0.06em', lineHeight:1.3 }}>{sc.name.toUpperCase()}</div>
                  <div style={{ fontSize:9, color:'#ccc', lineHeight:1.6 }}>{sc.desc}</div>
                </button>
              )
            })}
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
            <div>
              ...
              <select value={state.jurisdiction} onChange={e => update({ jurisdiction:e.target.value })}
                style={{ width:'100%', padding:'9px 10px', background:'#0c120e', border:'0.5px solid #1a3028', color:'#8adaaa', fontSize:11, fontFamily:'JetBrains Mono, monospace', borderRadius:3, outline:'none' }}>
            ...
            <div>
              ...
              {state.difficulty && (
                <div style={{ marginTop:7, fontSize:9, color:'#5a9a70', lineHeight:1.6, paddingLeft:2 }}>
          ...
          <button
            ...
            style={{ width:'100%', padding:'14px', fontSize:12, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', border:`0.5px solid ${state.scenario?ac:'#1a3028'}`, color:state.scenario?'#080c0a':'#3a6a48', background:state.scenario?ac:'transparent', cursor:(state.scenario&&!initLoading)?'pointer':'not-allowed', fontFamily:'JetBrains Mono, monospace', borderRadius:3, opacity:initLoading?0.6:1 }}>
      ...
      {/* BOTTOM STATUS BAR */}
      <div style={{ borderTop:'0.5px solid #0f1f14', background:'rgba(8,12,10,0.97)', padding:'8px 32px', flexShrink:0 }}>
        <div style={{ maxWidth:1200, margin:'0 auto', width:'100%', display:'flex', gap:0 }}>
          {[
            ...
            <div key={k} style={{ display:'flex', flexDirection:'column', gap:2, paddingRight:24, marginRight:24, borderRight: i < arr.length-1 ? '0.5px solid #0f1f14' : 'none' }}>
