...
const FLASH_CARD_COACHING_PATTERNS = [
  ...
  /\b(?:has|have|is|are)\s+not\s+(?:yet\s+)?been\s+(?:established|activated|stood up|coordinated|initiated)\b/i,
  /\bno\s+(?:formal|unified|joint|coordinated)\s+(?:public information posture|message|command framework|coordination structure|traffic management|access control perimeter|joint information center)\b/i,
]

const FLASH_CARD_MAX_SENTENCES = 2
const FLASH_CARD_MAX_CHARS = 360

function tightenFlashCardText(value='') {
  const text = String(value || '').replace(/\s+/g, ' ').trim()
  if (!text) return ''

  const sentences = text.match(/[^.!?]+[.!?]+|[^.!?]+$/g) || [text]
  let tightened = sentences
    .map(sentence => sentence.trim())
    .filter(Boolean)
    .slice(0, FLASH_CARD_MAX_SENTENCES)
    .join(' ')
    .trim()

  if (tightened.length <= FLASH_CARD_MAX_CHARS) return tightened

  const clipped = tightened.slice(0, FLASH_CARD_MAX_CHARS + 1)
  const breakAt = clipped.lastIndexOf(' ')
  tightened = clipped.slice(0, breakAt > 220 ? breakAt : FLASH_CARD_MAX_CHARS).replace(/[,:;\s]+$/, '')
  return `${tightened}.`
}
...
function neutralizeFlashCard(value='') {
  ...
  if (kept.length) return tightenFlashCardText(kept.join(' '))

  const source = flashCardSource(text)
  return tightenFlashCardText(`${source} reports an unresolved operating condition. Supporting information remains incomplete.`)
}
...
async function renderAarPdfV8(filename, rawText) {
  ...
  // They are parsed from the report text only after the standard two-page shared AAR,
  // so the proven cover and core layout remain unchanged.
  const appendixHeadingMap = {
    'ROLE CONTRIBUTION': { accent:colors.cyan, icon:'pin' },
    'ROLE DECISIONS': { accent:colors.blue, icon:'clipboard' },
    'ROLE COORDINATION': { accent:colors.teal, icon:'chain' },
    'ROLE COMMUNICATIONS': { accent:colors.cyan, icon:'comms' },
    'INDIVIDUAL STRENGTHS': { accent:colors.green, icon:'check' },
    'INDIVIDUAL GAPS': { accent:colors.red, icon:'warn' },
    'ROLE RECOMMENDATIONS': { accent:colors.amber, icon:'bulb' },
    'OVERVIEW': { accent:colors.cyan, icon:'pin' },
    'CROSS-ROLE DECISIONS': { accent:colors.blue, icon:'clipboard' },
    'TEAM INTEGRATION': { accent:colors.teal, icon:'chain' },
    'COORDINATION PATTERNS': { accent:colors.cyan, icon:'comms' },
    'FACILITATOR PRIORITIES': { accent:colors.amber, icon:'bulb' },
  }
  const appendixHeadings = new Set(Object.keys(appendixHeadingMap))
  const appendixGroups = []
  let appendixGroup = null
  let appendixSection = null
  pdfSafe(rawText).split('\n').forEach(lineText => {
    const trimmed = lineText.trim()
    const upper = trimmed.toUpperCase()
    if (/^INDIVIDUAL ROLE AAR\s*[—-]/i.test(trimmed) || upper === 'FACILITATOR AAR') {
      if (appendixSection && appendixGroup) appendixGroup.sections.push(appendixSection)
      if (appendixGroup) appendixGroups.push(appendixGroup)
      appendixGroup = { title:trimmed, sections:[] }
      appendixSection = null
      return
    }
    if (!appendixGroup) return
    if (appendixHeadings.has(upper)) {
      if (appendixSection) appendixGroup.sections.push(appendixSection)
      appendixSection = { label:upper, lines:[] }
      return
    }
    if (appendixSection && !/^[=-]{8,}$/.test(trimmed) && !/^NEXUS EOC\s*-\s*nexuseoc\.com$/i.test(trimmed)) {
      appendixSection.lines.push(lineText)
    }
  })
  if (appendixSection && appendixGroup) appendixGroup.sections.push(appendixSection)
  if (appendixGroup) appendixGroups.push(appendixGroup)
  ...
  appendixGroups.forEach(group => {
    ...
    const closeAppendixPage = () => {
      ...
      finishPage(appendixPageNo)
      appendixPageNo += 1
      pageStarted = false
    }
    beginAppendixPage(false)
    group.sections.forEach(section => {
      const style = appendixHeadingMap[section.label] || { accent:colors.cyan, icon:'clipboard' }
      const body = section.lines.join('\n').trim() || 'Not captured in this exercise record.'
      const wrapped = []
      String(body).split(/\n\s*\n/).forEach(part => {
        if (part.trim()) {
          wrapped.push(...wrap(part, 7.85, appendixBodyW))
          wrapped.push('')
        }
      })
      if (wrapped.length && wrapped[wrapped.length - 1] === '') wrapped.pop()
      const chunks = []
      for (let i = 0; i < wrapped.length; i += maxAppendixLines) chunks.push(wrapped.slice(i, i + maxAppendixLines))
      if (!chunks.length) chunks.push(['Not captured in this exercise record.'])
      chunks.forEach((chunk, chunkIndex) => {
        const chunkBody = chunk.join('\n')
        const estimatedH = Math.max(58, 38 + paraHeight(chunkBody, appendixBodyW, 7.85, 9.35))
        if (currentY - estimatedH < appendixMinY) {
          closeAppendixPage()
          beginAppendixPage(true)
        }
        currentY = drawCard(
          contentX,
          currentY,
          contentW,
          `${section.label}${chunkIndex ? ' (continued)' : ''}`,
          chunkBody,
          style.accent,
          style.icon,
          7.85,
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      const savedState = saved ? { ...defaultState, ...JSON.parse(saved) } : null
      if (teamRoom) {
        const sameActiveSession = savedState?.screen === 'game' && savedState?.teamMode && savedState?.roomCode === teamRoom.toUpperCase()
        setState(sameActiveSession
          ? savedState
          : { ...defaultState, ...(savedState || {}), screen:'team-lobby', teamEntryMode:'join', roomCode:teamRoom.toUpperCase() })
        return
      }
